...
Author: Sijie Hu
Date: 20/03/2024
Description: Rewrite the `forward` function of YOLODetector_MM to support multimodal inputs during export.
Specifically:
    ln 54, 55: `batch_inputs` -> `batch_inputs1`, `batch_inputs2`
    ln 93: batch_inputs -> (batch_inputs1, batch_inputs2)
"""
...
@FUNCTION_REWRITER.register_rewriter(
    'mmyolo.models.detectors.yolo_detector_mm.YOLODetector_MM.forward')
def single_stage_detector__forward(self,
                                   batch_inputs1: torch.Tensor,
                                   batch_inputs2: torch.Tensor,
                                   data_samples: OptSampleList = None,
                                   mode: str = 'tensor',
                                   **kwargs) -> ForwardResults:
    """Rewrite `forward` for default backend to support multimodal inputs.

    Support configured dynamic/static shape for model input and return
    detection result as Tensor instead of numpy array.

    Args:
        batch_inputs1 (Tensor): Inputs with shape (N, C, H, W).
        batch_inputs2 (Tensor): Inputs with shape (N, C, H, W).
    ...
    img_shape = torch._shape_as_tensor(batch_inputs1)[2:]
    ...
    return __forward_impl(self, (batch_inputs1, batch_inputs2), data_samples=data_samples)
